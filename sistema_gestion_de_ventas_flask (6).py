...
def nuevo():
    ...
    """, (
        ...
        data['domicilio'], data['localidad'], data['telefono'],
        data['email'], 1
    ))
...
# EDITAR
@persona_bp.route('/editar/<int:id>', methods=['POST'])
def editar(id):
    data = request.form
    ...
    cursor.execute("""
        UPDATE Personas SET
        CUIT=?, CONDIVA=?, RAZONSOCIAL=?, DOMICILIO=?,
        LOCALIDAD=?, TELEFONO=?, EMAIL=?
        WHERE idPersona=?
    """, (
        data['cuit'], data['condiva'], data['razon'], data['domicilio'],
        data['localidad'], data['telefono'], data['email'], id
    ))
...
def eliminar(id):
    ...
    cursor.execute("UPDATE Personas SET ESTADO=0 WHERE idPersona=?", (id,))
